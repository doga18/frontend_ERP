...
const Home = () => {
  ...
  // Selectors
  const { dataResume, dataResume1, error, message, loading } = useSelector(
    (state: RootState) => state.summaryResume
  );
  // Detalhadamento de clientes
  const {
    total: countClientsResponse,
  ...
  // UseEffect
  useEffect(() => {
    dispatch(getAllClientCount({ limit: 1, page: 1 }));
  ...
  useEffect(() => {
    if (!loading && dataResume) {
      // Verificando se o dataResume tem dados válidos e não uma lista vazia
      if (dataResume.data === undefined || dataResume.data.length === 0) {
        //console.log('Nenhum dado encontrado no dataResume.');
        return;
      }
      ...
      const mappedData = (dataResume.data as resumeStatusResponse[]).map(
        (item) => ({
          name: item.status ?? "",
        ...
        <div className="min-h-screen w-full ">
          ...
          <section id="visionSystem">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <DashboardCard
                ...
                value={countClients ? countClients : 0}
              />
              <DashboardCard
          ...
          <section id="recharts pizza BarChart" className="mb-3">
            <div className="grid grid-cols-1 md:grid-cols-2 text-center items-center justify-center gap-6">
              <div className="bg-cyan-50 rounded-2xl shadow-md p-4 hover:shadow-xl transition-all duration-300 ease-in-out">
                <h3 className="text-lg font-semibold mb-2 text-gray-900">
                  Distribuição de OSs
                </h3>
                <GraphicPizza
                  resumeByStatus={resumeByStatus}
              ...
              <div className="bg-cyan-50 rounded-2xl shadow-md p-4 hover:shadow-xl transition-all duration-300 ease-in-out">
                <h3 className="text-lg font-semibold mb-2 text-gray-800">
                  Equipamentos mais atendidos e status
                </h3>
          ...
          <section id="notify" className='mb-6 my-5'>
            <div className="bg-amber-100 rounded-2xl shadow-md p-4">
