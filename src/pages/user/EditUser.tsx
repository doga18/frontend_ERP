...
import type { userDataList, userDataEdit } from "../../interfaces/AuthUserInterface";
...
const EditUser = (props: PropsEditUser) => {
  ...
  const handleNewPassword = () => {
    ...
    const baseString = userOnEdit?.name + Date.now().toString();
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // Montando o Objeto
    const newData: userDataEdit = {
      userId: userOnEdit?.userId.toString(),
      // Verificando se o nome é diferente do atual porém ser diferente de caracteres vazias e com comprimento de no minimo 3
      name: name !== '' || name !== userOnEdit?.name || name.length >= 3 ? name: userOnEdit?.name,
      lastname: lastname !== '' || lastname !== userOnEdit?.lastname ? lastname: userOnEdit?.lastname,
      email: email !== '' || email !== userOnEdit?.email ? email: userOnEdit?.email,
      avaiable: avaiable !== userOnEdit?.avaiable ? avaiable : userOnEdit?.avaiable,
      roleId: role !== userOnEdit?.roleId ? (role || userOnEdit?.roleId) : userOnEdit?.roleId,
    }
    // Verificando se a senha foi resetada para incluir no objeto, caso ela esteja preenchida.
    if(passwordHash.length >= 6 && passwordHash.length <= 8){
      const newDataWithPassword: userDataEdit = {
        ...newData,
        password: passwordHash
      }
      // Enviando para o redux com a senha
      dispatch(updateDataUser(newDataWithPassword));
    }
    // Enviando para o redux sem a senha
    dispatch(updateDataUser(newData));
  };
  // UseEffects
  useEffect(() => {
    // Limpando a hash de senha
    setPasswordHash('');
    if (props.userData){      
      setUserOnEdit(props.userData);
      setAvaiable(props.userData.avaiable);
    } 
            ...
            <form onSubmit={handleSubmit}>
              <div className="flex-col items-center justify-center">
                ...
                <p className="text-gray-950 my-1 montserrat pb-5 text-justify">
                  ...
                  de ID: {userOnEdit?.userId ?? ""} criado em:{" "}
                  {new Date(userOnEdit?.createdAt ?? "").toLocaleDateString(
                    "pt-BR"
                  )}{" "}
                  e que econtra-se agora como{" "}
                  <span
                    className={`${
                      userOnEdit?.avaiable
                        ? "text-green-600"
                  ...
                  >
                    {userOnEdit?.avaiable ? "Ativo" : "Inativo"}
                  </span>
                  , Lembrando que o cargo atual do usuário é{" "}
                  <span className="font-bold">
                    {userOnEdit?.roleId === 1 ? (
                      ...
                      <>
                        <span className="text-red-600">Proprietário</span>
                      </>
                    ) : userOnEdit?.roleId === 3 ? (
                      <>
                        <span className="text-red-600">Gerente</span>
                      </>
                    ) : userOnEdit?.roleId === 4 ? (
                      <>
                      ...
                      <>
                        <span className="text-green-600">Fornecedor</span>
                      </>
                    )}
                    .
                  </span>
                      ...
                      >
                        {name ? (
                          ...
                          <>
                            ...
                            <span className="text-red-600 capitalize ms-1">
                              {userOnEdit?.name}
                            </span>
                          </>
                        )}
